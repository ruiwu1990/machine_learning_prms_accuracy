$(document).ready(function(){
	...
	$('#get_error_result_id_GBT').on('click',function(){
		  $.ajax({
		    type: 'GET',
		    url: '/api/gb_tree_data',
		    success: function (data) {
		      var received_json = JSON.parse(data);
		      $('#result_info_id_GBT').html('<div style="margin-left: 60px;">The original rmse is: <span style="color:blue;font-weight: bold;">'+received_json['original_rmse']+'</span><br>'+
	...
	function draw_line_chart(o_p_list,i_p_list,o_list){
		...
		function drawChart() {
	        ...
	        var chart = new google.visualization.LineChart(document.getElementById('line_chart_id_GBT'));
